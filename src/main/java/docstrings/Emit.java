    ...
    public static String emitDocsJson(OpenAPI api, boolean noImports, boolean noWrapping) throws Exception {
        ...
        if (api != null && api.paths != null && api.paths.pathItems != null) {
            for (Map.Entry<String, PathItem> entry : api.paths.pathItems.entrySet()) {
                ...
                if (pi.get != null) pathMap.put("get", createSnippet(pi.get, noImports, noWrapping));
                if (pi.post != null) pathMap.put("post", createSnippet(pi.post, noImports, noWrapping));
                if (pi.put != null) pathMap.put("put", createSnippet(pi.put, noImports, noWrapping));
                if (pi.delete != null) pathMap.put("delete", createSnippet(pi.delete, noImports, noWrapping));
                if (pi.patch != null) pathMap.put("patch", createSnippet(pi.patch, noImports, noWrapping));
                if (pi.options != null) pathMap.put("options", createSnippet(pi.options, noImports, noWrapping));
                if (pi.head != null) pathMap.put("head", createSnippet(pi.head, noImports, noWrapping));
                if (pi.trace != null) pathMap.put("trace", createSnippet(pi.trace, noImports, noWrapping));
