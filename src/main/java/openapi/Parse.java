...
public class Parse {
    ...
    public static OpenAPI fromString(String content) throws IOException {
        try {
            ...
            if (root.has("info")) {
                ...
                api.info = new Info();
                if (infoObj.has("title")) api.info.title = infoObj.getString("title");
                if (infoObj.has("version")) api.info.version = infoObj.getString("version");
            ...
            if (root.has("paths")) {
                ...
                for (String pathKey : pathsObj.keySet()) {
                    ...
                    if (piObj.has("get")) pi.get = parseOperation(piObj.getJSONObject("get"));
                    if (piObj.has("post")) pi.post = parseOperation(piObj.getJSONObject("post"));
                    if (piObj.has("put")) pi.put = parseOperation(piObj.getJSONObject("put"));
                    if (piObj.has("delete")) pi.delete = parseOperation(piObj.getJSONObject("delete"));
                    if (piObj.has("patch")) pi.patch = parseOperation(piObj.getJSONObject("patch"));
    ...
    private static Operation parseOperation(JSONObject obj) {
        ...
        if (obj.has("operationId")) op.operationId = obj.getString("operationId");
        if (obj.has("summary")) op.summary = obj.getString("summary");
        if (obj.has("description")) op.description = obj.getString("description");
        ...
        if (obj.has("requestBody")) {
            ...
            if (rbObj.has("description")) reqBody.description = rbObj.getString("description");
            if (rbObj.has("required")) reqBody.required = rbObj.getBoolean("required");
        ...
        if (obj.has("responses")) {
            ...
            for (String rKey : respObj.keySet()) {
                ...
                if (ro.has("description")) r.description = ro.getString("description");
                if (ro.has("content")) {
    ...
    private static java.util.List<Object> parseParameters(JSONArray arr) {
        ...
        for (int i = 0; i < arr.length(); i++) {
            ...
            if (pObj.has("name")) p.name = pObj.getString("name");
            if (pObj.has("in")) p.in = pObj.getString("in");
            if (pObj.has("description")) p.description = pObj.getString("description");
            if (pObj.has("required")) p.required = pObj.getBoolean("required");
            if (pObj.has("schema")) {
                Schema schema = new Schema();
