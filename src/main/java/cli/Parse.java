		...
		while (lm.find()) {
			...
			if (line.startsWith("Info Object title: ")) {
				...
				if (line.contains(" name: ")) {
					...
					if (dIdx > nIdx)
						s.name = line.substring(nIdx, dIdx);
					else
						s.name = line.substring(nIdx);
					if (s.name.isEmpty())
						s.name = null;
				}
				...
				if (api.servers != null && !api.servers.isEmpty()) {
					...
					if (!desc.isEmpty())
						sv.description = desc;
					if (!enums.isEmpty())
						sv.enumValues = Arrays.asList(enums.split(", "));
				...
				if (lastParsedSchema != null) {
					Matcher m = Pattern.compile("  Discriminator propertyName=(.*?) mapping=(.*?) defaultMapping=(.*)")
							.matcher(line);
					if (m.matches()) {
						Discriminator d = new Discriminator();
						String pName = m.group(1);
						if (!pName.isEmpty() && !pName.equals("null"))
							d.propertyName = pName;
						...
						if (!mappingStr.isEmpty() && !mappingStr.equals("null")) {
							...
							for (String pair : mappingStr.split(",")) {
								...
								if (kv.length == 2)
									d.mapping.put(kv[0], kv[1]);
							}
						}
						String dMapping = m.group(3);
						if (!dMapping.isEmpty() && !dMapping.equals("null"))
							d.defaultMapping = dMapping;
				...
				if (lastParsedSchema != null) {
					Matcher m = Pattern
							.compile("  XML name=(.*?) namespace=(.*?) prefix=(.*?) attribute=(.*?) wrapped=(.*)")
							.matcher(line);
					if (m.matches()) {
						XML x = new XML();
						...
						if (attr.equals("true"))
							...
							x.attribute = false;
						String wrapped = m.group(5);
						if (wrapped.equals("true"))
							x.wrapped = true;
						else if (wrapped.equals("false"))
							x.wrapped = false;
				...
				java.util.regex.Matcher m = java.util.regex.Pattern.compile(
						"Component securitySchemes (\\S+) type=([^ ]+) scheme=([^ ]+) in=([^ ]+) name=([^ ]+) bearerFormat=([^ ]+) openIdConnectUrl=([^ ]+) oauth2MetadataUrl=([^ ]+) deprecated=([^ ]+)(?: description=\\\"(.+?)\\\"?)?")
						.matcher(line);
				if (m.matches()) {
					...
					if (!type.equals("-"))
						sc.type = unescape(type);
					String scheme = m.group(3);
					if (!scheme.equals("-"))
						sc.scheme = unescape(scheme);
					String in = m.group(4);
					if (!in.equals("-"))
						sc.in = unescape(in);
					String name = m.group(5);
					if (!name.equals("-"))
						sc.name = unescape(name);
					String bearerFormat = m.group(6);
					if (!bearerFormat.equals("-"))
						sc.bearerFormat = unescape(bearerFormat);
					String openIdConnectUrl = m.group(7);
					if (!openIdConnectUrl.equals("-"))
						sc.openIdConnectUrl = unescape(openIdConnectUrl);
					String oauth2MetadataUrl = m.group(8);
					if (!oauth2MetadataUrl.equals("-"))
						sc.oauth2MetadataUrl = unescape(oauth2MetadataUrl);
					String deprecated = m.group(9);
					if (deprecated.equals("true"))
						sc.deprecated = true;
					String desc = m.group(10);
					if (desc != null)
						sc.description = unescape(desc);
				...
				if (api.components != null && api.components.securitySchemes != null) {
					Matcher m = Pattern.compile("^Component securitySchemesFlowScope (\\S+) (\\S+) (\\S+) (.*)$")
							.matcher(line);
					if (m.find()) {
						...
						if (sc != null && sc.flows != null) {
							...
							if (flowType.equals("implicit"))
								flow = sc.flows.implicit;
							else if (flowType.equals("password"))
								flow = sc.flows.password;
							else if (flowType.equals("clientCredentials"))
								flow = sc.flows.clientCredentials;
					...
					if (parts.length >= 7) {
						...
						if (sc != null) {
							...
							if (flowType.equals("implicit"))
								...
								sc.flows.password = flow;
							else if (flowType.equals("clientCredentials"))
								sc.flows.clientCredentials = flow;
							else if (flowType.equals("authorizationCode"))
								sc.flows.authorizationCode = flow;
							else if (flowType.equals("deviceAuthorization"))
								sc.flows.deviceAuthorization = flow;
						}
					}
				}
				...
				if (pi == null) {
					pi = new PathItem();
					api.paths.pathItems.put(currentPath, pi);
				}
				...
				if (currentOp != null)
					currentOp.operationId = line.substring(17);
			} else if (line.startsWith("    Description: ")) {
				if (currentOp != null)
					currentOp.description = line.substring(17);
				...
				if (currentOp != null) {
					...
					if (parts.length > 1 && !parts[1].isEmpty())
						currentOp.externalDocs.description = parts[1];
				}
			} else if (line.startsWith("Callback: ")) {
				if (currentOp != null) {
					if (currentOp.callbacks == null)
						currentOp.callbacks = new HashMap<>();
				...
				if (currentOp != null) {
					...
					if (!line.isEmpty())
						((RequestBody) currentOp.requestBody).description = line;
				}
				...
				if (currentOp != null && currentOp.requestBody instanceof RequestBody) {
					...
					if (parts.length > 2 && rb.content != null && rb.content.containsKey(parts[0])) {
						...
						if (enc.prefixEncoding == null)
							enc.prefixEncoding = new ArrayList<>();
				...
				if (currentParameter != null) {
					...
					if (colIdx != -1) {
						...
						if (dashIdx != -1 && eqIdx != -1) {
							...
							if (val.equals("null"))
								val = null;
							if (currentParameter.examples == null)
								currentParameter.examples = new HashMap<>();
				...
				if (currentOp != null) {
					if (currentOp.parameters == null)
						currentOp.parameters = new ArrayList<>();
				...
				if (currentOp != null) {
					...
					if (code.equals("default"))
						...
						currentOp.responses.statusCodes.put(code, currentResponse);
				}
				...
				if (currentOp != null && currentOp.responses != null) {
					...
					if (parts.length > 2) {
						Response r = parts[0].equals("default")
								? (Response) currentOp.responses.defaultResponse
								: (Response) currentOp.responses.statusCodes.get(parts[0]);
				...
				if (currentResponse != null) {
					if (currentResponse.headers == null)
						currentResponse.headers = new HashMap<>();
					Header h = new Header();
				...
				if (currentResponse != null) {
					if (currentResponse.links == null)
						currentResponse.links = new HashMap<>();
