...
public class CliMainTest {
    ...
    public void testWasiVirtualRoot() throws Exception {
        ...
        runMain(new String[]{"from_openapi", "to_sdk", "-i", specFile.getAbsolutePath(), "-o", new File(wasiDir, "out").getAbsolutePath()});
    }
    ...
    public void testProcessInMemoryDocsJson() throws Exception {
        ...
        String payload = "{\"command\":[\"to_docs_json\", \"--no-imports\", \"--no-wrapping\"],\"files\":{\"spec.json\":\"" + spec.replace("\"", "\\\"") + "\"}}";
    ...
    public void testProcessInMemoryFromOpenapi() throws Exception {
        ...
        runMain(new String[]{"process_in_memory", "{\"command\":[\"from_openapi\", \"to_sdk_cli\", \"--tests\"],\"files\":{\"spec.json\":\"" + escapedSpec + "\"}}"});
        runMain(new String[]{"process_in_memory", "{\"command\":[\"from_openapi\", \"to_sdk\", \"--tests\"],\"files\":{\"spec.json\":\"" + escapedSpec + "\"}}"});
        runMain(new String[]{"process_in_memory", "{\"command\":[\"from_openapi\", \"to_server\"],\"files\":{\"spec.json\":\"" + escapedSpec + "\"}}"});
        runMain(new String[]{"process_in_memory", "{\"command\":[\"from_openapi\", \"to_orm\", \"--no-github-actions\", \"--no-installable-package\"],\"files\":{\"spec.json\":\"" + escapedSpec + "\"}}"});
    }
    ...
    public void testFromOpenapi() throws Exception {
        ...
        runMain(new String[]{"from_openapi", "to_sdk_cli", "-i", specFile.getAbsolutePath(), "-o", outDir.getAbsolutePath(), "--tests"});
        runMain(new String[]{"from_openapi", "to_sdk", "-i", specFile.getAbsolutePath(), "-o", outDir.getAbsolutePath(), "--tests"});
        runMain(new String[]{"from_openapi", "to_server", "-i", specFile.getAbsolutePath(), "-o", outDir.getAbsolutePath()});
        runMain(new String[]{"from_openapi", "to_orm", "-i", specFile.getAbsolutePath(), "-o", outDir.getAbsolutePath(), "--no-github-actions", "--no-installable-package"});
        runMain(new String[]{"from_openapi", "to_sdk", "--input-dir", tempDir.getAbsolutePath(), "-o", outDir.getAbsolutePath()});
    ...
    public void testToDocsJson() throws Exception {
        ...
        runMain(new String[]{"to_docs_json", "-i", specFile.getAbsolutePath(), "-o", outFile.getAbsolutePath(), "--no-imports", "--no-wrapping"});
